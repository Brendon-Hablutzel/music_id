...
fn main() -> Result<(), String> {
    ...
    return_string_err!(stdin().read_line(&mut piece_id));
    let piece_id = return_string_err!(piece_id.trim_end().parse::<i32>());

    println!("Enter the new path:");
    return_string_err!(stdin().read_line(&mut new_path));
    ...
    let res = return_string_err!(diesel::update(music.filter(id.eq(piece_id)))
